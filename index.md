# Neil Berry

Electrical Engineer (Entry-Level) — Quality/Manufacturing • Test/Validation • Embedded HW Integration  
Oklahoma City, OK • Open to Remote/Hybrid  
Email: Luzcidmusic@gmail.com • Phone: 845-667-0301  
LinkedIn: [add link] • Portfolio Repo: [add link]

## About
B.S. Electrical Engineering (SUNY New Paltz, 2012). Former LED product engineer at Environmental Lights.
Built real-world electronics systems using programmable LEDs and Raspberry Pi control (sensing + actuation).
Strong in testing, documentation, troubleshooting/root-cause analysis, and customer-facing technical support.

## Featured Projects
- [Automated Plant Watering System (Raspberry Pi + Soil Moisture + Solenoids)](projects/watering-system.md)
- [Music-Reactive LEDs + Video-Driven LED Matrix](projects/led-video-matrix.md)
- [3× 150W 7-Band Spectrum LED Grow Lights](projects/grow-lights.md)

## Resume
assets/img/EE Resume.pdf
